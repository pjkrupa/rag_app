...
def test_table_creation(fake_configs):
    db = DatabaseManager(configs=fake_configs)
...
def test_create_user(fake_configs):
    db = DatabaseManager(configs=fake_configs)
    with db._get_conn() as conn:
        cursor = conn.cursor()
...
def test_check_user_valid(fake_configs):
    db = DatabaseManager(configs=fake_configs)
    db.create_user("test_user")
...
def test_check_user_invalid(fake_configs):
    db = DatabaseManager(configs=fake_configs)
    db.create_user("test_user")
    user_id = db.check_user("invalid_user")
    assert user_id is None

def test_create_chat(fake_configs):
    db = DatabaseManager(configs=fake_configs)
    db.create_user("test_user")
    user_id = db.check_user("test_user")
...
def test_insert_message(fake_configs):
    db = DatabaseManager(configs=fake_configs)
    db.create_user("test_user")
    user = User(configs=fake_configs, db=db, user_name="test_user")
...
def test_get_messages(fake_configs):
    db = DatabaseManager(configs=fake_configs)
